...
function findFreePort(startPort = 3000, maxAttempts = 20): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    let attempts = 0;
    const tryPort = () => {
      if (attempts >= maxAttempts) { reject(new Error(`No free port found near ${startPort}`)); return; }
      attempts += 1;
...
function waitForServer(port: number, timeoutMs = 20000): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const attempt = () => {
      ...
      req.on('error', () => {
        if (Date.now() - start > timeoutMs) reject(new Error(`Server did not become ready on port ${port}`));
...
function startServer(port: number): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    if (isDev) {
      ...
      serverProcess = spawn(tsNodeBin, [serverEntry], { stdio: 'inherit', env, shell: process.platform === 'win32', cwd: process.cwd() });
    } else {
      const serverJs = path.join(__dirname, '..', 'server.js');
      serverProcess = spawn(process.execPath, [serverJs], { stdio: 'inherit', env });
    }
    if (!serverProcess) { reject(new Error('Failed to spawn server process')); return; }
    serverProcess.on('error', reject);
    serverProcess.on('exit', (code) => { if (code && code !== 0) console.error(`Server process exited with code ${code}`); });
...
function createWindow(port: number) {
  ...
  mainWindow.on('close', (event) => {
    if (tray && process.platform !== 'darwin') { event.preventDefault(); mainWindow?.hide(); }
  });
}
...
function createMenu() {
  const template: Electron.MenuItemConstructorOptions[] = [
    { label: 'File', submenu: [
      ...
      { type: 'separator' },
      { label: process.platform === 'darwin' ? 'Quit Accessible Terminal' : 'Exit', accelerator: process.platform === 'darwin' ? 'Cmd+Q' : 'Alt+F4', click: () => { cleanup(); app.quit(); } }
    ] },
    { label: 'Edit', submenu: [
    ...
    { label: 'Help', submenu: [
      { label: 'Open in Browser', click: () => shell.openExternal(`http://${HOST}:${selectedPort}`) },
      { label: 'About', click: () => dialog.showMessageBox({ type: 'info', title: 'About Accessible Terminal', message: 'Accessible Terminal', detail: 'A screen-reader-friendly PowerShell / shell interface.\n\nVersion 1.0.0' }) }
    ] }
  ];
  if (process.platform === 'darwin') template.unshift({ label: app.name, submenu: [
    { role: 'about' }, { type: 'separator' }, { role: 'services' }, { type: 'separator' }, { role: 'hide' }, { role: 'hideOthers' }, { role: 'unhide' }, { type: 'separator' }, { role: 'quit' }
  ] });
...
function createTray() {
  ...
  try {
    const candidates = [path.join(process.resourcesPath || '', 'assets', 'icon.png'), path.join(__dirname, '..', '..', 'assets', 'icon.png'), path.join(process.cwd(), 'assets', 'icon.png')];
    let iconPath = '';
    for (const candidate of candidates) if (fs.existsSync(candidate)) { iconPath = candidate; break; }
...
function cleanup() {
  if (serverProcess && !serverProcess.killed) {
    if (process.platform === 'win32') spawn('taskkill', ['/pid', String(serverProcess.pid), '/f', '/t'], { stdio: 'ignore' });
...
app.whenReady().then(async () => {
  try {
    ...
    dialog.showErrorBox('Startup Error', err instanceof Error ? err.message : String(err));
    cleanup();
    app.quit();
  }
  app.on('activate', () => {
